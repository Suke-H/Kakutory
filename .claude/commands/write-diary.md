...
**必須項目（引数がなければ対話的に質問）：**
- `date` : 日付（`yyyymmdd` 形式、例: `20260310`）
- `title` : 日記タイトル
...
`astro_src/pages/diary/{id}.md` が存在しない場合のみ、以下のテンプレートで作成してください：
...
`public/images/{id}/` が存在しない場合のみ、フォルダを作成してください（Bashで `mkdir -p public/images/{id}` を実行）。
